...
function pbr(scene:Scene,name:string,kit:TextureKit,tint:string,metallic:number,roughness:number,emissive?:string):PBRMaterial{
  const material=new PBRMaterial(name,scene);
  material.albedoTexture=kit.albedo;
  material.bumpTexture=kit.normal;
  material.bumpTexture.level=.75;
  ...
  material.environmentIntensity=.9;
  if(emissive&&kit.emissive){material.emissiveTexture=kit.emissive;material.emissiveColor=Color3.FromHexString(emissive);material.emissiveIntensity=1.2;}
  return material;
}
...
export function createMaterialLibrary(scene:Scene):MaterialLibrary{
  ...
  return{
    ...
    projectile:pbr(scene,"projectile",kits.metal,"#effcff",1,1),
